...
import com.playarea.MaxDiffInArray_TreeApproachDidntWork.BinaryTree.TreeNode;
 ...
 *
 */
public class MaxDiffInArray_TreeApproachDidntWork {
	...
	public static void main(String[] args) {
		...
		// ALGORITHM
		// 1. Traverse from right to left and create a binary tree
		// 2. While constructing the tree maintain a HashMap for each node and its'
		// reference so that we can jump to that particular node while calculating the
		// difference
		// 3. Start from root node of the binary tree created, and traverse to all left
		// nodes and find the max difference as in step #4.
		// 4. always maintain the max difference variable and update while

		int a[] = { 2,3,10,2,4,8,1};
		
		Map<Integer, TreeNode> map = createTree(a);
		int maxDiff = -1;
		int previousValue = a[a.length-1];
		for (int i = a.length - 2; i >= 0; i--) {
			int value = a[i];
			TreeNode currentNode = map.get(i);
			while (currentNode != null) {
				maxDiff = (maxDiff >= (previousValue - value)) ? maxDiff : (previousValue - value);
	...
	private static Map<Integer, TreeNode> createTree(int[] a) {
		Map<Integer, TreeNode> nodeReferenceMap = new HashMap<>();
		BinaryTree binaryTree = new BinaryTree();
		for (int i = a.length - 1; i >= 0; i--) {
			nodeReferenceMap.put(i, binaryTree.add(a[i]));
		}
	...
	public static class BinaryTree {
		...
		public TreeNode add(int value) {
			...
			if (root == null) {
				...
				if (value <= currentNodeReference.value) {
					currentNodeReference.left = node;
				} else {
					currentNodeReference.right = node;
				}

			}

			currentNodeReference = node;
			return node;
		}
